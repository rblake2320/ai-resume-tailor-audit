...
Every saved result preserves employment level, growth rate, annual openings, replacement openings, geography, source URL, as-of date, projection years, retrieval time, uncertainty statement, and wage amount/currency/period/unit when supplied. Missing required fields or a source date older than three years produces no trend classification. Decline never means “no jobs”; openings and replacement demand remain separate measures. Pasted projection records are labeled `user_supplied_unverified`; an official-looking URL is provenance supplied by the user, not proof that Resume Foundry fetched or authenticated the data.
...
O*NET v2 lookup is performed server-side with the deployment's `ONET_API_KEY` in the official `X-API-Key` header. Credentials are never accepted from or returned to the browser. The occupation overview endpoint provides the title, description, reported titles, and `updated.year/contents`; Resume Foundry does not imply that skills, tasks, knowledge, or technology subresources were fetched. Provider requests and responses are size-bounded, malformed or undated O*NET data fails closed, and tests inject official-shape fixtures rather than making live calls.

Training recommendations appear only when a resource maps to an explicit evidence gap. Each keeps provider/source, price and currency, duration, prerequisites, accessibility, accreditation, evidence quality, and as-of date, while remaining labeled user-supplied/unverified. Prices in different currencies are never ordered by their raw numeric amount because Resume Foundry performs no exchange-rate conversion. Rankings are scenarios, not promises of employment, wages, admission, certification, or fit.
