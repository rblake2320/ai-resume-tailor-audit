...
Override a scope with `RESUME_FOUNDRY_<SCOPE>_LIMIT` and `RESUME_FOUNDRY_<SCOPE>_WINDOW_MS`, replacing punctuation with underscores (for example `RESUME_FOUNDRY_FETCH_JOB_LIMIT`). Values are validated; invalid configuration fails closed.

This is a global, single-host overload/spend control. It is not user identity, tenant fairness, abuse attribution, or a multi-host/distributed limit. Public multi-user deployment still requires authenticated tenants and a shared production limiter at the gateway or durable service layer.
