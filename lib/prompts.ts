...
For requirement_evidence, enumerate every material mandatory requirement, preferred qualification, responsibility, and logistical constraint. Each evidence item MUST be one exact, contiguous, verbatim span copied from original_resume; never paraphrase, summarize, combine separate spans, or copy the XML entity spelling. Each tailoredText item MUST be one exact, contiguous visible-text span from the tailored resume or cover letter without Markdown formatting characters such as **. If unsupported, evidence and tailoredText MUST both be empty; use recommendation for an honest clarification question, adjacent skill, portfolio task, training/certification, or intentional omission. Never turn a recommendation into a claimed qualification.
