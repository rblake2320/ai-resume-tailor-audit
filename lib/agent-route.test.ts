...
let root = "";
beforeEach(async () => { root = await mkdtemp(path.join(tmpdir(), "foundry-route-")); process.env.RESUME_FOUNDRY_AGENT_STORE = path.join(root, "store.json"); process.env.RESUME_FOUNDRY_AGENT_API_TOKEN = "api-secret"; process.env.RESUME_FOUNDRY_AGENT_AUDIT_KEY = "test-only-audit-key-with-32-bytes-minimum"; });
afterEach(async () => { delete process.env.RESUME_FOUNDRY_AGENT_STORE; delete process.env.RESUME_FOUNDRY_AGENT_API_TOKEN; delete process.env.RESUME_FOUNDRY_AGENT_AUDIT_KEY; await rm(root, { recursive: true, force: true }); });
